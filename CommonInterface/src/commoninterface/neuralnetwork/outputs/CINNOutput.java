...
public abstract class CINNOutput implements Serializable {
	...
	public static Vector<CINNOutput> getNNOutputs(RobotCI robot, CIArguments arguments) {
		...
		for (int i = 0; i < outputs.getNumberOfArguments(); i++) {
			CINNOutput nnOutput = createOutput(robot,
					new CIArguments(outputs.getArgumentAsString(outputs.getArgumentAt(i)), true));
	...
	public static CINNOutput createOutput(RobotCI robot, CIArguments args) {
		CINNOutput output = (CINNOutput) CIFactory.getInstance(args.getArgumentAsString("classname"), robot, args);
		String label = args.getArgumentAsStringOrSetDefault("label", output.getClass().getSimpleName());
