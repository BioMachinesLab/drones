...
public class ClassSearchUtils implements Serializable {
	...
	* 
	*/
	private static final long serialVersionUID = -3110595010780634953L;
	...
	public static String getClassFullName(String className) {
		...
		if (names.size() == 0) {
			...
			throw new RuntimeException("Multiple implementations of class: " + className + " - " + names);
		}
		return names.get(0);
	}
	 ...
	 */
	private static void lookForNamesInDirectory(String className, String name, File dir, ArrayList<String> classNames) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (file.isDirectory()) {
				lookForNamesInDirectory(className, name + fileName + ".", file, classNames);
			} else {
				try {
	 ...
	 */
	private static void lookForNamesInArchive(String className, File archive, ArrayList<String> classNames) {
		...
		while (entries.hasMoreElements()) {
			...
			if (entryName.toLowerCase().endsWith(".class")) {
				try {
					...
					if (entryName.endsWith("." + className) && !entryName.equals(className)) {
