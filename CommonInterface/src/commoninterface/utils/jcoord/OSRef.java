...
public class OSRef implements Serializable {
	private static final long serialVersionUID = 5795498233155541163L;
	 ...
	 * "TG514131") and create a new OSRef object that represents that grid
	 * reference. The first character must be H, N, S, O or T. The second
	 * character can be any uppercase character from A through Z excluding I.
	 * 
	 * @param ref
	 *            a String representing a six-figure Ordnance Survey grid
	 *            reference in the form XY123456
	 ...
	 * OSGB36 datum. Note that, the LatLng object may need to be converted to
	 * the WGS84 datum depending on the application.
	 * 
	...
	public LatLon toLatLng() {
		...
		do {
			M = (b * OSGB_F0) * (((1 + n + ((5.0 / 4.0) * n * n) + ((5.0 / 4.0) * n * n * n)) * (phiPrime - phi0))
					- (((3 * n) + (3 * n * n) + ((21.0 / 8.0) * n * n * n)) * FastMath.sin(phiPrime - phi0)
							* FastMath.cos(phiPrime + phi0))
					+ ((((15.0 / 8.0) * n * n) + ((15.0 / 8.0) * n * n * n)) * FastMath.sin(2.0 * (phiPrime - phi0))
							* FastMath.cos(2.0 * (phiPrime + phi0)))
					- (((35.0 / 24.0) * n * n * n) * FastMath.sin(3.0 * (phiPrime - phi0))
							* FastMath.cos(3.0 * (phiPrime + phi0))));
			phiPrime += (N - N0 - M) / (a * OSGB_F0);
		} while ((N - N0 - M) >= 0.001);
		double v = a * OSGB_F0 * FastMath.pow(1.0 - eSquared * Util.sinSquared(phiPrime), -0.5);
		double rho = a * OSGB_F0 * (1.0 - eSquared) * FastMath.pow(1.0 - eSquared * Util.sinSquared(phiPrime), -1.5);
		double etaSquared = (v / rho) - 1.0;
		double VII = FastMath.tan(phiPrime) / (2 * rho * v);
		double VIII = (FastMath.tan(phiPrime) / (24.0 * rho * FastMath.pow(v, 3.0))) * (5.0
				+ (3.0 * Util.tanSquared(phiPrime)) + etaSquared - (9.0 * Util.tanSquared(phiPrime) * etaSquared));
		double IX = (FastMath.tan(phiPrime) / (720.0 * rho * FastMath.pow(v, 5.0))) * (61.0
				+ (90.0 * Util.tanSquared(phiPrime)) + (45.0 * Util.tanSquared(phiPrime) * Util.tanSquared(phiPrime)));
		double X = Util.sec(phiPrime) / v;
		double XI = (Util.sec(phiPrime) / (6.0 * v * v * v)) * ((v / rho) + (2 * Util.tanSquared(phiPrime)));
		double XII = (Util.sec(phiPrime) / (120.0 * FastMath.pow(v, 5.0))) * (5.0 + (28.0 * Util.tanSquared(phiPrime))
				+ (24.0 * Util.tanSquared(phiPrime) * Util.tanSquared(phiPrime)));
		double XIIA = (Util.sec(phiPrime) / (5040.0 * FastMath.pow(v, 7.0))) * (61.0
				+ (662.0 * Util.tanSquared(phiPrime)) + (1320.0 * Util.tanSquared(phiPrime) * Util.tanSquared(phiPrime))
				+ (720.0 * Util.tanSquared(phiPrime) * Util.tanSquared(phiPrime) * Util.tanSquared(phiPrime)));
		phi = phiPrime - (VII * FastMath.pow(E - E0, 2.0)) + (VIII * FastMath.pow(E - E0, 4.0))
				- (IX * FastMath.pow(E - E0, 6.0));
		lambda = lambda0 + (X * (E - E0)) - (XI * FastMath.pow(E - E0, 3.0)) + (XII * FastMath.pow(E - E0, 5.0))
				- (XIIA * FastMath.pow(E - E0, 7.0));
