...
public class GeoFenceCISensor extends ConeTypeCISensor {
	private static final long serialVersionUID = 5428190435329902157L;
	...
	private void updateCones() {
		...
		try {
			...
			for (int sensorNumber = 0; sensorNumber < numberSensors; sensorNumber++) {
				...
				// We have to subtract 90 degrees and negate because the math
				// functions were made
				// for JBotEvolver, in which the reference orientation is to the
				// east, not the north,
				// and the orientation increases counter-clockwise instead of
				// clockwise
