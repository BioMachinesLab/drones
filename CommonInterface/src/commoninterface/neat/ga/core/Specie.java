...
public abstract class Specie implements Comparable {
	private ArrayList<Chromosome> specieMembers;
	...
	public Specie(double threshold, int id) {
		this.specieMembers = new ArrayList<Chromosome>();
	...
	/**
	 * Ages the specie fitness age if the best has not increased in fitness or
	 * the average fitness as a whole has not increased.
	 *
	 */
	public void ageFitness() {
		...
		if (this.specieMembers.size() > 0) {
			for (i = 0; i < this.specieMembers.size(); i++) {
				totalFitness += this.specieMembers.get(i).fitness();
			}
	...
	/**
	 * Adds the given individual to the specie iff it is compatible with the
	 * fitest member.
	 * 
	 * @param specieMember
	...
	public ArrayList<Chromosome> specieMembers() {
	...
	/**
	 * Creates offspring from the specie. The offspring count is passed in and
	 * is dependant on the overall specie fitness.
	 * 
	...
	public Gene[] cloneGenes(Gene[] clonee) {
		...
		for (i = 0; i < clonee.length; i++) {
			if (clonee[i] instanceof NEATLinkGene) {
				// int innovationNumber, boolean enabled, int fromId, int toId,
				// double weight
				cloned[i] = new NEATLinkGene(((NEATGene) clonee[i]).getInnovationNumber(),
						((NEATLinkGene) clonee[i]).isEnabled(), ((NEATLinkGene) clonee[i]).getFromId(),
						((NEATLinkGene) clonee[i]).getToId(), ((NEATLinkGene) clonee[i]).getWeight());
				...
				cloned[i] = new NEATNodeGene(((NEATGene) clonee[i]).getInnovationNumber(),
						((NEATNodeGene) clonee[i]).id(), ((NEATNodeGene) clonee[i]).sigmoidFactor(),
						((NEATNodeGene) clonee[i]).getType(), ((NEATNodeGene) clonee[i]).bias());
