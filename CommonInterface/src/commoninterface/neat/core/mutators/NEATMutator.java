...
public class NEATMutator implements Mutator {
	private static final long serialVersionUID = -2184171887553481367L;
	...
	private Gene mutateFeature(NEATFeatureGene mutatee) {
		...
		if (perturbRandVal < this.pPerturb) {
			mutated = new NEATFeatureGene(mutatee.getInnovationNumber(),
					mutatee.geneAsNumber().doubleValue() + MathUtils.nextClampedDouble(-perturb, perturb));
		}
	...
	private Gene mutateLink(NEATLinkGene mutatee) {
		...
		if (perturbRandVal < this.pPerturb) {
			...
			mutated = new NEATLinkGene(mutatee.getInnovationNumber(), mutatee.isEnabled(), mutatee.getFromId(),
	...
	private Gene mutateNode(NEATNodeGene mutatee) {
		...
		if (perturbRandVal < this.pPerturb) {
			...
			mutated = new NEATNodeGene(mutated.getInnovationNumber(), mutated.id(), newSF, mutated.getType(),
					mutated.bias());
		}

		if (mutateBias < this.pMutateBias) {
			newBias += MathUtils.nextClampedDouble(-biasPerturb, biasPerturb);
			mutated = new NEATNodeGene(mutated.getInnovationNumber(), mutated.id(), mutated.sigmoidFactor(),
					mutated.getType(), newBias);
		}

		return (mutated);
	}

	private boolean linkIllegal(NEATNodeGene from, NEATNodeGene to, ArrayList<Gene> links) {
		...
		if ((to.getType() == NEATNodeGene.INPUT)) {
			...
			while (!illegal && (idx < links.size())) {
				...
				// if ((linkGene.getFromId() == from.id() && linkGene.getToId()
				// == to.id()) || ((to.getDepth() <= from.getDepth()) &&
				// !this.recurrencyAllowed)) {
	...
	private void mutateAddLink(Chromosome mutatee) {
		...
		int rIdx;
		int i = 0;
		ArrayList<Gene> links;
		ArrayList<Gene> nodes;
	...
	private void assignNeuronDepth(Gene[] nodeGenes, int depth, Chromosome mutated) {
		...
		NEATNodeGene node;
		// ArrayList nodeGenes =
		// this.findOutputNodes(this.candidateNodes(genes));
	...
	private Gene[] findSourceNodes(int nodeId, Gene[] genes) {
		...
		ArrayList<Gene> links = this.candidateLinks(genes, true);
		NEATLinkGene link;
		...
		for (i = 0; i < sourceNodes.length; i++) {
			sourceNodes[i] = sources.get(i);
		}

		return (sourceNodes);
	}

	private Gene[] findOutputNodes(ArrayList<Gene> nodes) {
	...
	private int findChosenIndex(NEATLinkGene chosen, Chromosome mutatee) {
		...
		while (i < mutateeSize && idx == -1) {
			if (genes[i] instanceof NEATLinkGene && ((NEATLinkGene) genes[i]).getFromId() == chosen.getFromId()
	...
	private ArrayList<Gene> candidateLinks(Gene[] genes, boolean statusImportant) {
		...
		for (i = 0; i < genes.length; i++) {
			...
			if (gene instanceof NEATLinkGene) {
				// if (!statusImportant || (statusImportant &&
				// ((NEATLinkGene)gene).isEnabled() &&
				// ((NEATLinkGene)gene).getFromId() !=
				// ((NEATLinkGene)gene).getToId())) {
	...
	private ArrayList<Gene> candidateNodes(Gene[] genes) {
