...
public class NEATSpecie extends Specie {
	...
	protected void adjustFitness() {
		ArrayList<Chromosome> members = this.specieMembers();
		int i;
		Chromosome member;
	...
	protected boolean isCompatable(Chromosome specieApplicant, Chromosome specieRepresentative) {
		...
		if (specieRepresentative == null) {
			...
			compatabilityScore = NEATSpecieManager.specieManager().compatibilityScore(specieApplicant,
					specieRepresentative, this.excessCoeff, this.disjointCoeff, this.weightCoeff);
	...
	protected Chromosome[] produceOffspring(int count, Mutator mut, ParentSelector selector, CrossOver xOver) {
		...
		ArrayList<Chromosome> unsorted = this.specieMembers();
		...
		try {
			...
			System.err.println("produceOffspring produced error:count:" + count + ":current iteration:" + i
					+ ":specie size:" + this.specieMembers().size());
			e.printStackTrace();
		}
	...
	/**
	 * @param youthThreshold
	 *            The youthThreshold to set.
	 */
