...
public class NEATGeneticAlgorithm implements GeneticAlgorithm {
	...
	public void createPopulation() {
		...
		this.pop = new NEATPopulation4J(popSize, initialChromoSize, this.descriptor.getInputNodes(),
				this.descriptor.getOutputNodes(), this.descriptor.featureSelectionEnabled(),
				this.descriptor.getExtraFeatureCount());
	...
	private void speciatePopulation(Chromosome[] currentGen) {
		...
		ArrayList<Specie> currentSpecieList;
		Specie specie;
		...
		for (i = 0; i < currentGen.length; i++) {
			if (!memberAssigned) {
				...
				while (!memberAssigned && j < currentSpecieList.size()) {
					specie = currentSpecieList.get(j);
				...
				if (!memberAssigned) {
					...
					// ((NEATChromosome)currentGen[i]).setSpecieId(specie.id());
					// cat.info("Created new specie, member assigned to specie "
					// + specie.id());
				}
			}
		...
		/*
		 * if (cat.isDebugEnabled()) { for (i = 0; i <
		 * this.specieList.specieList().size(); i++) { specie =
		 * (Specie)this.specieList.specieList().get(i); if (!specie.isExtinct()
		 * && specie.specieMembers().size() > 0) { cat.debug("Specie:" +
		 * specie.id() + ":size:" + specie.specieMembers().size() + ":age:" +
		 * ((NEATSpecie)specie).specieAge() + ":fAge:" + +
		 * specie.getCurrentFitnessAge() + ":best:" +
		 * specie.findBestMember().fitness()); } } }
		 */
	}
	...
	public void runEvolutionCycle(Chromosome[] currentGen) {
		...
		ArrayList<Specie> validSpecieList;
		...
		if ((this.discoveredBest == null)
				|| (this.genBest.fitness() >= this.discoveredBest.fitness() && !this.descriptor.isNaturalOrder())
				|| (this.genBest.fitness() <= this.discoveredBest.fitness() && this.descriptor.isNaturalOrder())) {
			// copy best
			this.discoveredBest = this.cloneChromosome(this.genBest);
		}
		// cat.info("Best Ever Raw:" + (this.discoveredBest.fitness()) + ":from
		// specie:" + ((NEATChromosome)this.discoveredBest).getSpecieId());
		// cat.debug("Best of Generation is:" + (this.genBest.fitness()) + "
		// specie " + ((NEATChromosome)this.genBest).getSpecieId());
		...
		if (this.descriptor.getCompatabilityChange() > 0) {
			if (validSpecieList.size() > this.descriptor.getSpecieCount()) {
				this.descriptor.setThreshold(this.descriptor.getThreshold() + this.descriptor.getCompatabilityChange());
					...
					&& (this.descriptor.getThreshold() > this.descriptor.getCompatabilityChange())) {
				this.descriptor.setThreshold(this.descriptor.getThreshold() - this.descriptor.getCompatabilityChange());
			}
		}
		this.eleCount++;
	...
	private Chromosome[] spawn() {
		...
		ArrayList<Specie> species = this.specieList
				.validSpecieList(((NEATChromosome) this.discoveredBest).getSpecieId());
		for (i = 0; i < species.size(); i++) {
			...
			if (i == (species.size() - 1)) {
				...
				// offSp = this.specieList.calcSpecieOffspringCount(specie,
				// this.descriptor.gaPopulationSize(), totalAvFitness);
			} else {
				offSpringCount = this.specieList.calcSpecieOffspringCount(specie, this.descriptor.gaPopulationSize(),
						totalAvFitness);
			}

			// cat.debug("Sp[" + specie.id() + "] Age:" +
			// ((NEATSpecie)specie).specieAge() + ":Offspring Sz:" +
			// offSpringCount + ":AvF:" + specie.getAverageFitness() + ":FAge:"
			// + specie.getCurrentFitnessAge() + ":BestF:" +
			// specie.findBestMember().fitness());
			if (offSpringCount > 0) {
				...
				// cat.debug("Specie " + specie.id() + ":size:" +
				// specie.specieMembers().size() + " produced no offspring.
				// Average fitness was " + specie.averageFitness() + " out of a
				// total fitness of " +
				// this.specieList.totalAvSpeciesFitness());
				specie.setExtinct();
			}
		}
