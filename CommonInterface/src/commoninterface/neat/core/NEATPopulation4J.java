 ...
 * @author MSimmerson
 *
 *         Contains a population of NEAT chromosomes that are used to describe a
 *         NEAT neural network.
 */
public class NEATPopulation4J implements Population {
	private static final long serialVersionUID = 4828437191002030162L;
	...
	public void createPopulation() {
		...
		Chromosome[] templates = db.initialiseInnovations(this.popSize, this.inputs, this.outputs,
				this.featureSelection, this.extraFeatureCount);
	...
	private Chromosome individualFromTemplate(Chromosome template) {
		...
		for (i = 0; i < templateGenes.length; i++) {
			if (templateGenes[i] instanceof NEATNodeGene) {
				...
				individualGenes[i] = new NEATNodeGene(nodeGene.getInnovationNumber(), nodeGene.id(),
						MathUtils.nextPlusMinusOne(), nodeGene.getType(), MathUtils.nextDouble());
			} else if (templateGenes[i] instanceof NEATLinkGene) {
				linkGene = (NEATLinkGene) templateGenes[i];
				individualGenes[i] = new NEATLinkGene(linkGene.getInnovationNumber(), true, linkGene.getFromId(),
						linkGene.getToId(), MathUtils.nextPlusMinusOne());
