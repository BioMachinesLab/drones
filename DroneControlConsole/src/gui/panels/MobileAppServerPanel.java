...
import network.mobileAppServer.ServerObserver;

public class MobileAppServerPanel extends JPanel implements ServerObserver {
	private static final long serialVersionUID = -6896535178671903709L;
	...
	public MobileAppServerPanel(DroneGUI gui) {
		this.mobileAppServer = ((DroneControlConsole) (gui.getConsole())).getMobileAppServer();
	...
	private void toggleButton() {
		if (mobileAppServer.isRunning()) {
			...
			if (portNumberTextfield.getText().matches("^[0-9]{1,5}$")
				...
				mobileAppServer.startServer(Integer.parseInt(portNumberTextfield.getText()));
			} else {
				JOptionPane.showMessageDialog(null, "Invalid port number!", "Input Error", JOptionPane.ERROR_MESSAGE);
			}
		}
	}
	...
	public synchronized void setOnlineServer() {
		try {
			...
			displayMessageArea.setText("Running on " + InetAddress.getLocalHost().getHostAddress() + ":"
