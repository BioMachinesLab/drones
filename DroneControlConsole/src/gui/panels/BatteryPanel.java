...
public class BatteryPanel extends UpdatePanel {
	private static final long serialVersionUID = -3816396108438276095L;
	...
	public BatteryPanel() {
		...
		for (int i = 0; i < CELL_QUANTITY; i++) {
			voltageMeters[i] = new JProgressBar((int) (MIN_VOLTAGE * VOLTAGE_MULTIPLIER),
	...
	private class MyProgressUI extends BasicProgressBarUI {
		...
		protected void paintDeterminate(Graphics g, JComponent c) {
			...
			if (progressBar.isStringPainted()) {
				...
				Point placement = getStringPlacement(g, voltageStr, 0, 30, c.getWidth() / 2, getHeight() / 2);
				...
				g.drawString(voltageStr, placement.x, placement.y + fm.getAscent() - 8);
				g.drawString(percentStr, placement.x + 5, placement.y + fm.getAscent() + 8);
			}
		}
	}
	...
	public synchronized void displayData(BatteryMessage message) {
		...
		for (int i = 0; i < CELL_QUANTITY; i++) {
			voltageMeters[i].setValue((int) (batteryStatus.getCellsVoltages()[i] * VOLTAGE_MULTIPLIER));
		}
