...
public abstract class FieldTestScript extends Thread {
	 ...
	 * clearMapEntities
	 *
	 * GeoFence defineGeoFence (does not add to the Console's map) ArrayList
	 * <Waypoint> generateWaypointsInGeoFence (does not add to the Console's
	 * map)
	 *
	 */
	@Override
	...
	protected void startControllers(ArrayList<String> ips, CIArguments args) {
		BehaviorMessage msg = new BehaviorMessage(args.getArgumentAsString("type"), args.getCompleteArgumentString(),
				true, NetworkUtils.getHostname());
		deploy(msg, ips);
	}
	...
	protected ArrayList<Waypoint> generateWaypointsInGeoFence(GeoFence fence, int number, double minDistance,
			double maxDistance, long randomSeed) {
