...
public class CameraCaptureInput implements ControllerInput, MessageProvider {
	...
	public Message getMessage(Message request) {
		if(request instanceof InformationRequest && ((InformationRequest)request).getMessageTypeQuery().equals(InformationRequest.MessageType.CAMERA_CAPTURE)){
							...
							for(int i = 0 ; i < bytesRead - 1 ; i++) {
								if(content[i] == -1 && content[i+1] == -39) {
									...
									if(i+2 < bytesRead) {
										...
										baos.write( content, 0, originalBytes+1);
										overflowed = true;
										extraBytes.write(content, originalBytes, bytesRead-originalBytes);
