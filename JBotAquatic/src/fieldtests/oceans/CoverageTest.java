...
//		int robots = 50;
		...
		int faultTypes[] = new int[]{-60,-30,-15,0,10,20,30,40,50};
						...
						for(double area : chosenAreas) {
							for(String type : allTypes) {
								for(String controller : files) {
									...
									args = new String[]{"--robots","+numberofrobots="+robots,"--environment","+width="+size+",height="+size+",wallsdistance="+size/2+",steps="+time+",griddecay=0,distance="+startDistance/*,"--controllers","+angletolerance="+angle*/};
									...
									} else if(fault > robots) {
										continue;
									} else {
										s+=" --updatables +fault=(classname=ProbabilityFaultInjection,deadrobots="+fault+")";
									}
