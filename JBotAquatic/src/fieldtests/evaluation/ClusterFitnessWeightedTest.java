...
public class ClusterFitnessWeightedTest extends AvoidCollisionsFunction {
	private static final long serialVersionUID = -3973737307275836103L;
	...
	public void update(Simulator simulator) {
		...
		if (lastSteps) {
			...
			fitness = (numRobots - totalClusters / totalNorm) / (numRobots - 1);
		}
				...
				for (int j = i + 1; j < clusters.size() && !merged; j++) {
					...
					for (int ri = 0; ri < c1.size() && !merged; ri++) {
						for (int rj = 0; rj < c2.size() && !merged; rj++) {
							...
							} else if (r1.getPosition().distanceTo(r2.getPosition()) - r1.getRadius()
