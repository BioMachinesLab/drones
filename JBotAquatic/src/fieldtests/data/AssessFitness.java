...
public class AssessFitness {
	...
	public static double getRealFitness(Experiment exp, long randomSeed, boolean gui) {
		...
		for (ToLogData d : exp.logs) {
			...
			setup.robots.get(position).setPosition(pos.x + setup.start.x - setup.firstPos.x,
					pos.y + setup.start.y - setup.firstPos.y);
	...
	public static double getSimulatedFitness(Experiment exp, long randomSeed, boolean gui) {
		...
		for (ToLogData d : exp.logs) {
			...
			// RobotLocation rl = Setup.getRobotLocation(d);
			//
			// commoninterface.mathutils.Vector2d pos =
			// CoordinateUtilities.GPSToCartesian(rl.getLatLon());
			//
			// r.setPosition(pos.x+setup.start.x-setup.firstPos.x,
			// pos.y+setup.start.y-setup.firstPos.y);
			// double orientation = 360 - (rl.getOrientation() - 90);
			...
			while (Math.abs(stepTime.getMillis() - currentTime.getMillis()) >= 100) {
				...
				// AquaticDrone aq = (AquaticDrone)
				// setup.getRobot("192.168.3.2");
				// ControllerCIBehavior b =
				// (ControllerCIBehavior)aq.getActiveBehavior();
	...
	public static void compareFitness(Experiment exp, long randomSeed, boolean gui) {
		...
		for (int i = 0; i < simSamples; i++) {
			...
			Coverage temp = new Coverage(
					new Arguments("resolution=" + setupReal.resolution + ",distance=5,decrease=0,min=20,max=25"));
			CoverageTracer tempTracer = new CoverageTracer(
					new Arguments("bgcolor=0-0-0-0,min=20,max=25,color=1,folder=temp/" + exp.toString()));
			...
			if (coverage && setupSim.eval instanceof CoverageFitnessTest) {
				...
				// COVERAGE
				CoverageTracer simCoverageTracer = new CoverageTracer(new Arguments(
						"drawgeofence=0,gradient=0,altcolor=0-0-0-255,png=1,maincolor=46-203-206-255,bgcolor=0-0-0-0,folder=coverage/"
								+ exp.toString() + "_sim"));
				...
				CoverageTracer realCoverageTracer = new CoverageTracer(new Arguments(
						"drawgeofence=0,gradient=0,altcolor=255-0-0-255,png=1,maincolor=46-203-206-255,bgcolor=0-0-0-0,folder=coverage/"
								+ exp.toString() + "_real"));
				// CoverageTracer realCoverageTracer = new CoverageTracer(new
				// Arguments("png=1,sequence=1,linewidth=3,snapshotfrequency=5,scale=10,gradient=0,altcolor=255-0-0-255,png=1,maincolor=255-255-000-255,bgcolor=0-0-0-0,folder=coverage/"+exp.toString()+"_real"));
			...
			if (pathTracer) {
				PathTracer realPathTracer = new PathTracer(new Arguments(
						"drawgeofence=0,linewidth=3,folder=path,bgcolor=0-0-0-0,altcolor=0-0-0-255,maincolor=255-000-000-255,fade=1,name="
								+ exp.toString() + "_real"));
				setupReal.sim.addCallback(realPathTracer);
				// PathTracer simPathTracer = new PathTracer(new
				// Arguments("drawgeofence=1,linewidth=3,folder=path,bgcolor=0-0-0-0,altcolor=0-0-0-255,maincolor=255-000-000-255,fade=0,name="+exp.toString()+"_sim"));
				// setupSim.sim.addCallback(simPathTracer);
			}

			// DISPERSION ADAPTIVE
			// setupReal.sim.addCallback(new PathTracer(new
			// Arguments("drawgeofence=0,png=0,snapshotfrequency=5,timestart=0,timeend=600,linewidth=3,bgcolor=0-0-0-0,altcolor=255-0-0-255,maincolor=255-0-000-255,fade=1,folder=path/"+exp.toString()+"_1")));
			// setupReal.sim.addCallback(new PathTracer(new
			// Arguments("drawgeofence=0,png=0,snapshotfrequency=5,timestart=600,timeend=1800,linewidth=3,bgcolor=0-0-0-0,altcolor=255-0-0-255,maincolor=255-0-000-255,fade=1,folder=path/"+exp.toString()+"_2")));
			// setupReal.sim.addCallback(new PathTracer(new
			// Arguments("drawgeofence=0,png=0,snapshotfrequency=5,timestart=1800,timeend=2400,linewidth=3,bgcolor=0-0-0-0,altcolor=255-0-0-255,maincolor=255-0-000-255,fade=1,folder=path/"+exp.toString()+"_3")));

			// PATROL ADAPTIVE
			// setupReal.sim.addCallback(new PathTracer(new
			// Arguments("drawgeofence=0,png=0,snapshotfrequency=5,timestart=0,timeend=1200,linewidth=3,bgcolor=0-0-0-0,altcolor=255-0-0-255,maincolor=255-0-000-255,fade=1,folder=path/"+exp.toString()+"_1")));
			// setupReal.sim.addCallback(new PathTracer(new
			// Arguments("drawgeofence=0,png=0,snapshotfrequency=5,timestart=1200,timeend=1800,linewidth=3,bgcolor=0-0-0-0,altcolor=255-0-0-255,maincolor=255-0-000-255,fade=1,folder=path/"+exp.toString()+"_2")));
			// setupReal.sim.addCallback(new PathTracer(new
			// Arguments("drawgeofence=1,png=0,snapshotfrequency=5,timestart=1800,timeend=3600,linewidth=3,bgcolor=0-0-0-0,altcolor=255-0-0-255,maincolor=255-0-000-255,fade=1,folder=path/"+exp.toString()+"_3")));
			// setupReal.sim.addCallback(new PathTracer(new
			// Arguments("drawgeofence=0,png=0,snapshotfrequency=5,timestart=3600,timeend=5400,linewidth=3,bgcolor=0-0-0-0,altcolor=255-0-0-255,maincolor=255-0-000-255,fade=1,folder=path/"+exp.toString()+"_4")));
			// setupReal.sim.addCallback(new PathTracer(new
			// Arguments("drawgeofence=0,png=0,snapshotfrequency=5,timestart=5400,timeend=6600,linewidth=3,bgcolor=0-0-0-0,altcolor=255-0-0-255,maincolor=255-0-000-255,fade=1,folder=path/"+exp.toString()+"_5")));

			// AGG WP
			// setupReal.sim.addCallback(new PathTracer(new
			// Arguments("drawgeofence=0,png=0,snapshotfrequency=5,timestart=0,timeend=600,linewidth=3,bgcolor=0-0-0-0,altcolor=255-0-0-255,maincolor=255-0-000-255,fade=1,folder=path/"+exp.toString()+"_1")));
			// setupReal.sim.addCallback(new PathTracer(new
			// Arguments("drawgeofence=0,png=0,snapshotfrequency=5,timestart=0,timeend=1200,linewidth=3,bgcolor=0-0-0-0,altcolor=255-0-0-255,maincolor=255-0-000-255,fade=1,folder=path/"+exp.toString()+"_2")));
			// setupReal.sim.addCallback(new PathTracer(new
			// Arguments("drawgeofence=0,png=0,snapshotfrequency=5,timestart=0,timeend=1800,linewidth=3,bgcolor=0-0-0-0,altcolor=255-0-0-255,maincolor=255-0-000-255,fade=1,folder=path/"+exp.toString()+"_3")));
			// setupReal.sim.addCallback(new PathTracer(new
			// Arguments("drawgeofence=0,png=0,snapshotfrequency=5,timestart=0,timeend=2400,linewidth=3,bgcolor=0-0-0-0,altcolor=255-0-0-255,maincolor=255-0-000-255,fade=1,folder=path/"+exp.toString()+"_4")));
			...
			for (ToLogData d : exp.logs) {
						...
						if (step % 10 == 0) {
							for (Robot r : setupSim.sim.getRobots()) {
								...
								if (currentLatLonSim.get(r.getId()) == null) {
									...
									totalDistanceSim[r.getId()] += current.distanceInKM(currentLatLonSim.get(r.getId()))
											* 1000;
									currentLatLonSim.put(r.getId(), current);
								}
							...
							for (Robot r : setupReal.sim.getRobots()) {
								...
								if (currentLatLonReal.get(r.getId()) == null) {
									...
									totalDistanceReal[r.getId()] += current
											.distanceInKM(currentLatLonReal.get(r.getId())) * 1000;
									currentLatLonReal.put(r.getId(), current);
								}
							}
						}
				...
				if (exp.activeRobot != -1 && !d.ip.endsWith("." + exp.activeRobot)) {
					...
					Vector2d vecStopped = CoordinateUtilities
							.GPSToCartesian(setupSim.getShiftedLatLon(rlStopped.getLatLon()));
				...
				setupReal.robots.get(position).setPosition(pos.x + setupReal.start.x - setupReal.firstPos.x,
						pos.y + setupReal.start.y - setupReal.firstPos.y);

				// temp.addPoint(setupReal.sim,setupReal.robots.get(position).getPosition(),
				// d.temperatures[1]);
				// tempTracer.setCoverage(temp.getCoverage(),
				// setupReal.resolution);
				...
				FieldTest.writer.println(expid + " " + step + " " + position + " "
						+ (pos.x + setupReal.start.x - setupReal.firstPos.x) + " "
						+ (pos.y + setupReal.start.y - setupReal.firstPos.y) + " " + Math.toRadians(orientation));
				...
				// for(Waypoint wp :
				// GeoFence.getGeoFences((AquaticDroneCI)setupSim.sim.getRobots().get(0)).get(0).getWaypoints())
				// {
				// pos = CoordinateUtilities.GPSToCartesian(wp.getLatLon());
				// System.out.println(pos);
				// }
			...
			// }
			//
			// if(getDistanceToTarget(setupSim.getRobot("."+exp.activeRobot)) <
			// distanceToTarget) {
			// stopRun = true;
			// }
			// }
			...
			// count++;
			// double meters =
			// endLatLon.get(ss).distance(startLatLon.get(ss))*1000.0;
	...
	private static void startControllers(Experiment exp, Setup setup) {
		...
		try {
			...
			BehaviorMessage bm = new BehaviorMessage(readArguments.getArgumentAsString("type"),
					readArguments.getCompleteArgumentString(), true, "dude");
			...
			for (int i = 0; i < setup.robots.size(); i++) {
				...
				if (exp.activeRobot != -1) {
					...
					if (exp.controllerName.contains("hierarchical") && i == setup.robots.size() - 1) {
						...
						BehaviorMessage preprogbm = new BehaviorMessage(preprogArgs.getArgumentAsString("type"),
								preprogArgs.getCompleteArgumentString(), true, "dude");
						...
						readArguments2.setArgument("ip" + drone.getNetworkAddress(),
								(0.5 + 0.5 / (setup.robots.size() - 1 - 1) * i));
						bm = new BehaviorMessage(readArguments2.getArgumentAsString("type"),
								readArguments2.getCompleteArgumentString(), true, "dude");
						drone.processInformationRequest(bm, null);
					}
				}
			}
