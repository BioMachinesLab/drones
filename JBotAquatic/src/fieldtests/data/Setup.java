...
class Setup {
	...
	public Setup(Experiment exp, long randomSeed, boolean gui, boolean simulation) {
		...
		// real environment
		// hash.put("--environment", new
		// Arguments("classname=EmptyEnvironment,width=150,height=150,steps="+exp.timeSteps,true));

		// test environment for simulations
		hash.put("--environment", new Arguments(
				"classname=environment.TestHierarchicalMissionEnvironment,enemydistance=30,width=200,height=200,steps=5001,deploytime=0,onboardrange=40,simple=1"));
		...
		sim = new Simulator(randomSeed, hash);
		...
		// String className =
		// hash.get("--evaluation").getArgumentAsString("classname");
		...
		/*
		 * for(Updatable u : sim.getCallbacks()) if(u instanceof WaterCurrent)
		 ...
		 * 
		 * sim.addCallback(new WaterCurrent(new
		 * Arguments("fixedspeed=1,angle=-45,maxspeed="+val)));
		 */
		if (gui) {
