...
public class CompareFitnessField extends Thread {
	...
	private ArrayList<LogData> getData(String experiment, int controller, int robots, int sample) {
		...
		try {
			...
			Scanner sBcast = new Scanner(
					new File("compare/bcast/preset_" + experiment + controller + "_" + sample + ".txt"));
	...
	public void run() {
		...
		real.sim = new Simulator(1, hash);
		sim.sim = new Simulator(1113, hash);
