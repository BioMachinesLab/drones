...
public class AvoidEnvironment extends BoundaryEnvironment {
	private static final long serialVersionUID = 7031922172066854179L;
	...
	public void setup(Simulator simulator) {
		...
		for (int i = 0; i < otherRobots; i++) {
			...
			for (int j = 0; j < 4; j++) {
				...
				Vector2d pos = new Vector2d(wallsDistance * 1.25 * Math.cos(angle),
						wallsDistance * 1.25 * Math.sin(angle));
				wpPos.add(pos);
			}
			...
			AquaticDrone other = new AquaticDrone(simulator,
					new Arguments("diameter=1,gpserror=1.0,commrange=40,avoiddrones=0"));
