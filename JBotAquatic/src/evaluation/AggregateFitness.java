...
public class AggregateFitness extends AvoidCollisionsFunction {
	private static final long serialVersionUID = 1517105395851622656L;
	...
	public void update(Simulator simulator) {
		...
		// System.out.println(simulator.getTime()+" MEAN "+meanDistance+"
		// FITNESS "+fitness+" STARTING "+startingDistance+" CURRENT
		// "+currDistance+" ROBOTS "+simulator.getRobots().size());
	...
	private double calculateDistCM(Collection<Robot> robots, Simulator sim) {
		...
		// Iterator<PhysicalObject> i =
		// sim.getEnvironment().getAllObjects().iterator();
		//
		// while(i.hasNext()) {
		...
		// sim.getEnvironment().addStaticObject(new LightPole(sim, "lp",
		// centreMass.x, centreMass.y, 1));
