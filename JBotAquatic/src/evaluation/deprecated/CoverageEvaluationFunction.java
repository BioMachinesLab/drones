...
public class CoverageEvaluationFunction extends EvaluationFunction {
	private static final long serialVersionUID = 1588140825637777416L;
	private boolean isSetup = false;
	private double[][] coverage;
	private double resolution = 1;
	private double width = 5, height = 5;
	private double decrease = 1.0 / (10 * 100);// 1000 steps to go from 1.0 to
												// 0.0
				...
				if (insideLines(r.getPosition(), simulator)) {
					...
					for (int y = pMinY; y < pMaxY; y++) {
						...
						for (int x = pMinX; x < pMaxX; x++) {
							...
							// double px = (x -
							// coverage[y].length/2)*resolution;
