...
public class LawnMowerController extends Controller {
	private static final long serialVersionUID = -6241104962059002315L;
	...
	public void controlStep(double time) {
		...
		if (fence == null) {
			...
			for (int i = 0; i < nLanes; i++) {
				...
				Waypoint currentWp1 = new Waypoint("wp" + (currentIndex++),
						CoordinateUtilities.cartesianToGPS(new Vector2d(currentX, currentY)));
				...
				Waypoint currentWp2 = new Waypoint("wp" + (currentIndex++),
						CoordinateUtilities.cartesianToGPS(new Vector2d(currentX, currentY)));
