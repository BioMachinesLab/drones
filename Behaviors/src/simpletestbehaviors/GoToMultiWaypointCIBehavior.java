...
public class GoToMultiWaypointCIBehavior extends CIBehavior {
	...
	public void start() {
		if (defineWPs) {
			...
			distanceW = fence.getWaypoints().get(0).getLatLon().distanceInKM(fence.getWaypoints().get(1).getLatLon())
					* 1000;
			distanceH = fence.getWaypoints().get(1).getLatLon().distanceInKM(fence.getWaypoints().get(2).getLatLon())
					* 1000;
	...
	public void step(double timestep) {
		...
		// CoordinateUtilities.distanceInMeters(drone.getGPSLatLon(),
		// wp.getLatLon());
		// if(Math.abs(currentDistance) < distanceTolerance){
		// prevWP = wp;
		// wp = null;
		...
		double coordinatesAngle = CoordinateUtilities.angleInDegrees(drone.getGPSLatLon(), wp.getLatLon());
